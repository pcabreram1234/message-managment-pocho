...
const UniqueOrSaveMessageModal = ({
  message,
  cbShowModal,
  saveMessage,
  showModal,
  ...
  const API_SCHEDULED_MESSAGE =
    import.meta.env.VITE_API_URL +
    import.meta.env.VITE_API_URL_ROUTER +
    "configuration/scheduleCustomMessage";
  ...
  const handleOk = () => {
    ...
    submitData(API_SCHEDULED_MESSAGE, payload, "POST")
      .then((resp) => {
  ...
  return (
    ...
    >
      ...
      <Descriptions title="More Info" column={1}>
        ...
        >
          By selecting one or more contacts, the message(s) will be saved to be
          sent automatically every day at the current time, from the start date
...
export default UniqueOrSaveMessageModal;
